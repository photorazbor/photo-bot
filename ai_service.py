...
ПРАВИЛА ПРОВЕРКИ:
- Если тема «Горизонт и геометрия»: горизонт должен быть достаточно ровным (небольшой наклон в 1-2 градуса допустим, если вертикали в порядке). Явный завал (видно глазом) — это НЕВЫПОЛНЕНИЕ. Вертикали должны быть близки к вертикальным.
