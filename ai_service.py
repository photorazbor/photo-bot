...
def analyze_photo(image_bytes: bytes, course_topic: str = None) -> dict:
    ...
    if course_topic:
        system_prompt = f"""Ты проверяешь домашнее задание студента на курсе фотографии.
Тема задания: {course_topic}

Твоя задача:
1. Проверь, выполнено ли задание по теме.
2. ОТДЕЛЬНО отметь, что получилось хорошо (свет, эмоция, цвет, момент) — даже если задание провалено.
3. Будь поддерживающим, но честным. Не хвали за то, чего нет.

Формат: ТОЛЬКО JSON.
{{
  "error_type": "good_shot если выполнено, иначе topic_error",
  "title": "Задание: {course_topic}",
  "what_is_wrong": "Конкретно по теме задания: что не так. Если всё ок — напиши 'Задание выполнено правильно'.",
  "how_to_fix": "Как исправить именно по теме. Если всё ок — напиши 'Продолжай в том же духе!'.",
  "pro_tip": "Один совет по теме задания.",
  "praise": "Что получилось ХОРОШО в кадре, независимо от выполнения задания. Найди хотя бы одну деталь: свет, цвет, эмоцию, идею. Поддержи ученика.",
  "drawings": []
}}"""
    else:
