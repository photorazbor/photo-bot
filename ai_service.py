...
- Заполнение кадра и фон --- ВАЖНО: отличай «мусор» (случайные объекты, которые портят кадр: урны, провода, случайные прохожие) от «антуража» (интерьер, уличная сцена, детали, создающие атмосферу и историю). Если фон живой и добавляет колорит --- НЕ называй его ошибкой. Критикуй только явный мусор.
ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА (делай всегда, даже если других ошибок нет): внимательно осмотри зону вокруг головы и плеч каждого человека. Нет ли там объекта, который визуально «торчит» из человека? Фонарный столб, дерево, ветка, знак, угол здания, столб забора, труба — если такой объект пересекает голову или плечи, это ОШИБКА, указывай её обязательно. error_type = "fill_frame". В what_is_wrong опиши, какой именно объект торчит. В drawings добавь circle или line на этот объект.
Если рекомендуешь кадрирование --- ОБЯЗАТЕЛЬНО добавляй crop_frame в drawings.
