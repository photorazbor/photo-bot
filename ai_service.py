...
- Заполнение кадра и фон --- ВАЖНО: отличай «мусор» (случайные объекты, которые портят кадр: урны, провода, случайные прохожие, столбы, торчащие из головы, ветки, растущие из тела человека) от «антуража» (интерьер, уличная сцена, детали, создающие атмосферу и историю). Если фон живой и добавляет колорит --- НЕ называй его ошибкой. Критикуй только явный мусор.
ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА: посмотри, нет ли объектов, которые визуально «торчат» из человека --- столб из головы, ветка из плеча, провод из спины, урна за ухом. Если есть --- это ошибка fill_frame, укажи её обязательно.
