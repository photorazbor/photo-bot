...
async def handle_reference_photo(message: Message, user_id: int, image_bytes: bytes) -> bool:
    ...
    if step == "user_photo":
        ...
        prompt = (
            "Два изображения: ПЕРВОЕ — референс (стиль, сцена, свет), "
            "ВТОРОЕ — фото человека (лицо). Создай одно изображение: "
            "человек со ВТОРОГО фото в сцене ПЕРВОГО."
            "\n\n"
            ...
            "НЕ меняй брови, глаза, нос, губы, овал лица — только со ВТОРОГО. "
            ...
            "так же, как на референсе. Соблюдай единый источник света."
            "\n\n"
            "С ПЕРВОГО БЕРЁМ: свет, тени, фон, поза, атмосфера, "
            "цвет, тонирование, композиция, одежда, аксессуары."
        )
