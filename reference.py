...
async def handle_reference_photo(message: Message, user_id: int, image_bytes: bytes) -> bool:
    ...
    if step == "user_photo":
        ...
        prompt = (
            "Два изображения: ПЕРВОЕ — референс (стиль, сцена, свет, атмосфера). "
            "ВТОРОЕ — фото человека (лицо). "
            "Создай ОДНО изображение: человек со ВТОРОГО фото в сцене ПЕРВОГО. "
            "\n\n"
            "ЛИЦО — ТОЧНО СО ВТОРОГО ФОТО: "
            "форма лица, брови, глаза (цвет и разрез), нос, губы, "
            "веснушки (или их отсутствие), родинки, причёска, цвет волос, "
            "пол, возраст, телосложение. "
            "НЕ копируй лицо, брови, веснушки, цвет глаз, причёску с ПЕРВОГО. "
            "НЕ добавляй веснушки, если их нет на ВТОРОМ. "
            "НЕ меняй брови, глаза, нос, губы — только со ВТОРОГО. "
            "\n\n"
            "С ПЕРВОГО БЕРЁМ: свет, фон, позу, атмосферу, цветовую гамму, "
            "композицию, одежду, стиль съёмки. "
            "\n\n"
            "Результат: человек со ВТОРОГО фото как будто реально снялся "
            "в этой фотосессии. Лицо узнаваемое. Атмосфера как на референсе."
        )
