...
async def handle_reference_photo(message: Message, user_id: int, image_bytes: bytes) -> bool:
    ...
    if step == "user_photo":
        ...
        prompt = (
            "Перенеси стиль, композицию, позу, фон, свет и атмосферу "
            "с ПЕРВОГО изображения (референс) на ВТОРОЕ изображение (фото человека). "
            "Лицо и внешность человека со ВТОРОГО изображения должны быть "
            "вставлены в сцену с ПЕРВОГО. "
            "СОХРАНИ черты лица, причёску, пол, возраст, телосложение человека со второго фото. "
            "НЕ копируй лицо с референса — используй лицо со второго фото. "
            "НЕ добавляй новых людей. "
            "Результат — ОДНО изображение, как на референсе, "
            "но с человеком со второго фото. "
            "Стиль, освещение, цветовая гамма, композиция — как на референсе."
        )
