...
def _build_prompt(state: dict) -> str | None:
    ...
    face_lock = (
        ...
        "ВОЗРАСТ: сохрани ТОЧНЫЙ возраст людей как на исходном фото. "
        "НЕ старь людей — НЕ добавляй морщины, НЕ добавляй седину, НЕ делай кожу дряблой, НЕ добавляй усталость. "
        "НЕ молоди людей — НЕ убирай естественные возрастные черты, если они есть на фото. "
        "Лица должны выглядеть так же свежо и естественно, как на исходном фото. "
        "НЕ меняй черты лица, НЕ меняй национальность, НЕ стилизуй. "
        "Кожа должна быть ровной, чистой, свежей, но НЕ «пластиковой куклой». "
        "Можно слегка поправить причёску (уложить волосы) и добавить лёгкую естественную ретушь кожи. "
        "ЗАПРЕЩЕНО: подменять лицо, менять форму носа/глаз/губ, старить, молодить, добавлять или убирать людей. "
    ...
    accessories_lock = (
        ...
        "Сохрани ТОЛЬКО те украшения и аксессуары, которые есть на исходном фото. "
        "Если на руках нет колец — НЕ рисуй кольца. "
        "Если на руках нет браслетов — НЕ рисуй браслеты. "
        "Если нет серёг — НЕ рисуй серьги. "
        "Если нет цепочек — НЕ рисуй цепочки. "
        "Если нет часов — НЕ рисуй часы. "
        "Если украшения ЕСТЬ на фото — сохрани их в точности: то же место, тот же вид. "
        "ЗАПРЕЩЕНО добавлять любые украшения, которых не было на исходном фото. "
    )
    ...
    full = (
        ...
        f"{location_lock}"
        f"{atmosphere_lock}"
        f"Профессиональная фотография, кинематографичный свет, атмосферно, реалистично, но с лёгким винтажным налётом. "
        f"Размер: 1024x1024."
    )
    return full
