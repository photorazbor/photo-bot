...
def _build_prompt(state: dict) -> str | None:
    ...
    face_lock = (
        "КОЛИЧЕСТВО ЛЮДЕЙ — АБСОЛЮТНОЕ ПРАВИЛО, ВАЖНЕЕ ВСЕГО: "
        "Посчитай ТОЧНО, сколько людей на исходном фото. "
        "На новой картинке должно быть РОВНО СТОЛЬКО ЖЕ людей — ни больше, ни меньше. "
        "Один человек = ровно один. Двое = ровно двое. Шесть = ровно шесть. "
        "НЕ добавляй новых персонажей. "
        "НЕ убирай никого с исходного фото. "
        "НЕ заменяй людей. "
        "Все люди с исходного фото ОБЯЗАТЕЛЬНО должны быть на новой картинке. "
        "Если не уверен, сколько людей — пересчитай по головам. "
        "ЗАПРЕЩЕНО: добавлять случайных людей, силуэты, прохожих, фигуры вдалеке. "
        "ЗАПРЕЩЕНО: терять людей, делать группу меньше, чем на исходнике. "
        "ЗАПРЕЩЕНО: менять одного человека на другого. "
        "СХОДСТВО: люди должны быть УЗНАВАЕМЫ — те же лица, цвет волос, причёски, возраст, пол, телосложение. "
        "Сохрани форму лица, овал, разрез и цвет глаз, брови, нос, губы, подбородок. "
        "В рисованном стиле допускается упрощение черт лица, но узнаваемость сохрани. "
        "НЕ старь, НЕ молоди, НЕ меняй национальность. "
    )
    ...
    if is_art_style:
        realism_lock = ""
        # Усиленный блок для лиц — только в реалистичном стиле
        face_realism_lock = (
            "ЛИЦА — ПРИОРИТЕТ №1, ВАЖНЕЕ АТМОСФЕРЫ: "
            "СХОДСТВО с исходным фото — ГЛАВНОЕ. "
            "Если выбор между красивым кадром и точным лицом — "
            "выбирай ТОЧНОЕ ЛИЦО. "
            "\n\n"
            "СОХРАНИ у КАЖДОГО человека (по отдельности): "
            "форму лица, овал, лоб, брови (форма, толщина, цвет), "
            "глаза (форма, разрез, цвет), нос, губы, подбородок, уши, "
            "веснушки или их отсутствие, родинки, морщины, "
            "цвет волос, причёску, длину волос, "
            "пол, возраст, телосложение, тон кожи. "
            "\n\n"
            "ЗАПРЕЩЕНО УЛУЧШАТЬ ЛИЦА: "
            "НЕ ретушируй кожу — сохрани текстуру, поры, морщины. "
            "НЕ сглаживай складки. "
            "НЕ выравнивай тон кожи. "
            "НЕ делай лица симметричнее. "
            "НЕ делай брови гуще, темнее или ровнее. "
            "НЕ добавляй веснушки, если их нет на исходном. "
            "НЕ убирай родинки, если они есть. "
            "НЕ меняй цвет глаз, форму носа, губ, овал лица. "
            "НЕ старь и НЕ молоди. "
            "\n\n"
            "ЕСЛИ НЕСКОЛЬКО ЛЮДЕЙ: "
            "Каждому — ЕГО СОБСТВЕННОЕ лицо. "
            "НЕ смешивай черты между людьми. "
            "НЕ копируй черты у людей на фоне (прохожие, гости). "
            "\n\n"
            "ИНТЕГРАЦИЯ ЛИЦА В АТМОСФЕРУ: "
            "Лицо — узнаваемое, но освещено светом сцены. "
            "Тёплый свет гирлянд тонирует кожу тёплым. "
            "Тени от шапок, шарфов, капюшонов падают на лица естественно. "
            "Кожа сохраняет текстуру, но тон и свет — из сцены. "
            "Никаких резких границ между лицом и фоном. "
            "Лица — ЧАСТЬ сцены, а не наложенные портреты. "
            "\n\n"
            "АТМОСФЕРА: мягкий свет, лёгкий расфокус по краям кадра, "
            "боке от огоньков гирлянд, винтажные тёплые оттенки, "
            "уютная новогодняя атмосфера. "
            "Но лица — В ФОКУСЕ и УЗНАВАЕМЫ."
        )
        art_style_lock = ""
    ...
    # Усиленный блок для лиц — только для реалистичного стиля
    if not is_art_style:
        realism_face = face_realism_lock
    else:
        realism_face = ""
