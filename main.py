...
async def handle_flat_chfmt(callback: CallbackQuery):
    ...
    gen_format[user_id] = fmt
    gen_wish[user_id] = (
        f"Измени формат на {fmt}. "
        f"ПЕРЕРАСПРЕДЕЛИ предметы под новый формат: "
        f"разложи их гармонично, чтобы композиция заполнила весь кадр. "
        f"НЕ оставляй пустые края. "
        f"Можно менять расположение предметов. "
        f"Сохрани все предметы и стиль оформления."
    )
