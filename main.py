    ...
    try:
        ...
        if mode == "retry":
            ...
            if flat_lay_active.get(user_id, False):
                ...
                prompt = (
                    f"Создай НОВЫЙ вариант Flat Lay с этими же предметами. "
                    f"ПОЛНОСТЬЮ измени композицию: перемешай предметы, разложи их в другом порядке, под другими углами. "
                    f"Сгруппируй их по-новому, измени расстояния между ними. "
                    f"Используй другой декор, но в ТОМ ЖЕ стиле. "
                    f"Сохрани атмосферу и настроение текущего стиля. "
                    f"Фон должен соответствовать стилю, но может отличаться от предыдущего. "
...
async def handle_flat_chfmt(callback: CallbackQuery):
    ...
    gen_format[user_id] = fmt
    gen_wish[user_id] = (
        f"Создай НОВУЮ КОМПОЗИЦИЮ под формат {fmt}. "
        f"ПОЛНОСТЬЮ перемешай предметы: измени их расположение, порядок, углы. "
        f"Распредели предметы гармонично, чтобы они заполнили весь кадр. "
        f"НЕ оставляй пустые края. "
        f"Добавь новые декоративные элементы в том же стиле. "
        f"Композиция должна СУЩЕСТВЕННО отличаться от текущей. "
        f"Сохрани все предметы с фото и общий стиль оформления."
    )
...
FLAT_LAY_PROMPTS = {
    "cozy": "Создай атмосферный Flat Lay как из Pinterest. Полностью замени фон на тёплый деревянный стол. РАССТАВЬ предметы красиво и гармонично — можно менять их расположение, поворачивать, группировать. Добавь уютные детали: салфетку, веточку, тёплый свет. Распознай предметы и добавь УМЕСТНЫЙ декор. ЕСЛИ есть чашка — НАПОЛНИ её напитком. Сохрани все предметы с фото.",
    
    "minimal": "Создай минималистичный Flat Lay как из Pinterest. Полностью замени фон на белый. РАССТАВЬ предметы идеально — можно менять их расположение, создавать геометричную композицию. Много воздуха. Мягкий свет. ЕСЛИ есть чашка — НАПОЛНИ её напитком. Сохрани все предметы с фото.",
    
    "nature": "Создай природный Flat Lay как из Pinterest. Полностью замени фон на светлый мрамор. РАССТАВЬ предметы гармонично — можно менять их расположение. Добавь ЖИВЫЕ зелёные листья, эвкалипт. Мягкий дневной свет. ЕСЛИ есть чашка — НАПОЛНИ её напитком. Сохрани все предметы с фото. НЕ добавляй сухую траву.",
    
    "dark": "Создай элегантный тёмный Flat Lay как из Pinterest. Полностью замени фон на чёрный матовый. РАССТАВЬ предметы стильно — можно менять их расположение. Драматичный свет. ЕСЛИ есть чашка — НАПОЛНИ её напитком. Сохрани все предметы с фото.",
    
    "pastel": "Создай нежный пастельный Flat Lay как из Pinterest. Полностью замени фон на пастельный. РАССТАВЬ предметы красиво — можно менять их расположение. Добавь живые цветы. ЕСЛИ есть чашка — НАПОЛНИ её напитком. Сохрани все предметы с фото.",
}
