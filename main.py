    ...
    try:
        ...
        if mode == "retry":
            ...
            if flat_lay_active.get(user_id, False):
                prompt = (
                    f"Создай СОВЕРШЕННО НОВЫЙ Flat Lay как из Pinterest с этими предметами. "
                    f"Полностью замени фон на чистый и красивый. "
                    f"ПОЛНОСТЬЮ измени композицию: перемешай предметы, разложи их по-другому, под другими углами. "
                    f"Улучши внешний вид предметов: убери грязь, пятна. "
                    f"Используй другой декор в том же стиле. "
                    f"НЕ оставляй грязные поверхности, подтёки, ржавчину. "
                    f"Результат должен быть КРАСИВЫМ и СТИЛЬНЫМ. "
...
async def handle_flat_chfmt(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        f"Сделай результат КРАСИВЫМ и СТИЛЬНЫМ. "
        f"Сохрани все предметы с фото и общий стиль."
    )
...
async def handle_flat_refine_comp(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "Улучши внешний вид предметов: убери грязь, пятна. "
...
FLAT_LAY_PROMPTS = {
    ...
    "minimal": (
        ...
        "Добавь МИНИМУМ уместного декора: одну веточку, один камень, одну тень от предмета. "
    ...
    "nature": (
        ...
        "Улучши вид предметов: убери грязь, пятна. "
        "Добавь БОГАТЫЙ природный декор: "
        "ЖИВЫЕ зелёные листья, эвкалипт, цветы, веточки, камни, "
        "если это еда — зелень, орехи, ягоды; "
        "если косметика — лепестки цветов, листья. "
    ...
    "dark": (
        ...
        "камни, сухие ветки, тёмные ткани, металлические детали, "
        "если это кофе — тёмные зёрна, шоколад; "
        "если украшения — бархат, цепи. "
        "ЕСЛИ есть чашка — НАПОЛНИ её напитком. "
    ...
    "pastel": (
        ...
        "Улучши вид предметов: убери грязь, пятна. "
        "Добавь БОГАТЫЙ нежный декор: "
        "живые цветы, ленты, кружево, жемчуг, "
        "если косметика — кисти, зеркало, лепестки; "
        "если украшения — шёлк, цветы. "
        "ЕСЛИ есть чашка — НАПОЛНИ её напитком. "
