...
async def do_generation(user_id: int, chat_id: int, gen_type: str):
    ...
    try:
        ...
        prompt = (
            ...
            f"Если ноги выглядят обрезанными ниже колен или отсутствуют — дорисуй голени и стопы. НО если ноги спрятаны за объектом (стол, диван, камень, сумка) — это естественное перекрытие, не трогай этот объект и не пытайся его обойти или отрезать. "
            ...
            f"Если в кадре есть фрейминг — сделай его аккуратнее и эстетичнее, используя те же самые объекты, что и на исходном фото. Не заменяй их на другие. Просто сделай их форму изящнее, а расположение — гармоничнее. При необходимости слегка измени кадрирование, чтобы обрамление работало лучше. "
            f"Убери отвлекающие объекты, улучши свет и цвета. "
            f"НЕ меняй черты лица — сохрани их в точности как на исходном фото. "
            f"Сохрани все важные детали и объекты. Размер: {img_size}. При смене формата размести главный объект по правилам композиции: не прижимай к краю, оставь воздух по направлению взгляда, соблюдай правило третей и равновесие кадра."
        )
        ...
        if "framing" in error_type:
            prompt += f"Улучши фрейминг: если обрамление (ветки, арки, окна) гармоничное — сделай его аккуратнее. Если оно мешает или слишком громоздкое — аккуратно убери мешающую часть или замени на более изящное обрамление. {what_is_wrong} {how_to_fix}"
