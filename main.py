    ...
    try:
        ...
        if mode == "retry":
            ...
            if flat_lay_active.get(user_id, False):
                # Берём ПОЛНЫЙ промпт стиля и добавляем указание на новый вариант
                style_prompt = wish if wish and wish.lower() != "ок" else ""
                prompt = (
                    f"{style_prompt} "
                    f"Сделай СОВЕРШЕННО ДРУГОЙ вариант композиции. "
                    f"ПОЛНОСТЬЮ перемешай предметы, разложи их по-другому, под другими углами. "
