...
async def handle_int_light(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        "Довыровняй геометрию. "
        "Исправь края фото. "
        "НЕ меняй архитектуру: потолок, стены, ниши, перегородки. "
        "Улучши освещение: тёплый мягкий свет, убери тёмные углы. "
        "НЕ меняй светильники, мебель, декор."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_clean(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "Исправь края фото. "
        "НЕ меняй архитектуру: потолок, стены, ниши, перегородки. "
        "Убери мусор и беспорядок: скомканные вещи, сумки, провода, лишние предметы. "
        "НЕ меняй мебель, светильники."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_decor(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "Исправь края фото. "
        "НЕ меняй архитектуру: потолок, стены, ниши, перегородки. "
        "Добавь уместный декор: подушки, плед, вазу, растение. "
        "НЕ меняй светильники, люстры."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_full(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "Исправь края фото. "
        "НЕ меняй архитектуру: потолок, стены, ниши, перегородки. "
        "Сделай как для дорогого объявления: убери мусор, улучши свет, добавь декор. "
        "НЕ добавляй людей. "
        "НЕ меняй светильники, мебель."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
