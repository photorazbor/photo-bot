...
REQUISITES = (
    ...
    "📌 <b>В назначении платежа укажите:</b>\n"
    "• «Оплата за мини-курс по фотографии» — 490 ₽\n"
    "• «Оплата за пакет 5 генераций» — 99 ₽\n"
    "• «Оплата за пакет 20 генераций» — 249 ₽\n\n"
    "✅ После оплаты напишите мне — я проверю и открою доступ."
)
...
DONATE_LOGIN = "1515230"  # временно оставлен, пока не будет удалён весь код
...
def get_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if user_id == 456504792:
        ...
        buttons.append([InlineKeyboardButton(text="💛 5 улучшений --- 99 ₽", callback_data="show_requisites")])
        buttons.append([InlineKeyboardButton(text="💛 20 улучшений --- 249 ₽", callback_data="show_requisites")])
    ...
    if has_access(user_id) and user_mode.get(user_id) == "course":
        ...
        buttons.append([InlineKeyboardButton(text="💛 Поддержать на 100 ₽", callback_data="show_requisites")])
        buttons.append([InlineKeyboardButton(text="💛 Поддержать (любая сумма)", callback_data="show_requisites")])
...
async def handle_show_requisites(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer(
        f"{REQUISITES}",
        parse_mode="HTML",
