...
async def do_generation(user_id: int, chat_id: int, gen_type: str):
    ...
    try:
        ...
        prompt = (
            ...
            f"Дорисуй обрезанные края, улучши свет и цвета. "
            f"НЕ меняй черты лица. "
            f"Размер: {img_size}. "
            f"При смене формата размести главный объект по правилам композиции. "
        )
        
        if "horizon" in error_type:
            prompt += f"ОБЯЗАТЕЛЬНО выровняй горизонт. При повороте дорисуй недостающие участки. {what_is_wrong} {how_to_fix} "
