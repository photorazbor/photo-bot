...
async def handle_gen_go_retouch(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "Убери тёмные круги под глазами, покраснения, пигментные пятна, неровности тона. "
        "Кожа должна стать ЗАМЕТНО более гладкой и ровной, чем на исходном фото. "
        "Сохрани черты лица и текстуру — не делай пластиковую куклу. "
        "НЕ меняй позу, фон, свет, композицию, одежду. "
        "Только кожа: гладкая, ровная, чистая. "
        "Если результат не отличается от исходника — сделай ретушь сильнее."
    )
    await callback.answer("Запускаю генерацию...")
    await do_generation(user_id, callback.message.chat.id, gen_type)
    user_mode[user_id] = "free"
...
async def handle_hair(callback: CallbackQuery):
    ...
    prompt = (
        f"Сделай деловой портрет. "
        f"Белый фон. "
        f"Лицо анфас, плечи видны. "
        f"Нейтральное выражение. "
        f"Одежда: {outfit_name}. "
        f"Причёска: {hair_name}. "
        f"Студийный свет. "
        f"Сохрани черты лица."
    )
