    ...
    try:
        ...
        if mode == "retry":
            ...
            if flat_lay_active.get(user_id, False):
                # Для Flat Lay — более сильное указание
                prompt += (
                    " Сделай СОВЕРШЕННО ДРУГОЙ вариант композиции. "
                    "Полностью измени расположение предметов, декор, ракурс. "
                    "Не повторяй предыдущий результат. Используй другой подход к оформлению."
                )
            else:
                # Для обычных фото
                prompt += (
