...
FLAT_LAY_PROMPTS = {
    "cozy": (
        ...
        "НЕ добавляй чай или кофе, если предметы не связаны с едой или напитками. "
    ...
    "nature": (
        ...
        "Добавь уместные элементы: свежие фрукты, зелень, орехи, цветы, листья. "
...
async def handle_int_geometry(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "НЕ меняй светильники, люстры, бра, мебель. "
        "НЕ добавляй новую мебель или декор. "
        "Только исправь свет."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_clean(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "провода, пакеты, бутылки. "
        "Комната должна выглядеть чистой и опрятной. "
        "НЕ меняй мебель, светильники, планировку. "
        "Только убери лишнее."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_decor(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "растение в углу, картину на стену, статуэтку на полку. "
        "Декор должен соответствовать стилю комнаты. "
        "НЕ меняй светильники, люстры, бра. "
        "НЕ добавляй вентиляторы вместо люстр. "
        "Только добавь аккуратные декоративные элементы."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
...
async def handle_int_full(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        "НЕ меняй мебель, светильники, планировку. "
        "НЕ добавляй вентиляторы вместо люстр. "
        "Комната должна выглядеть чистой, уютной и готовой к продаже."
    )
    interior_active[user_id] = True
    gen_used_count[user_id] = 0
