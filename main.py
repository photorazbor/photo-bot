    ...
    try:
        ...
        if is_flat_lay:
            ...
            prompt = (
                ...
                f"НЕ добавляй новые объекты, людей, животных, которых не было на исходном фото. "
                f"ВАЖНО: сохрани стиль верхней одежды с исходного фото. "
                f"Если дорисовываешь низ (брюки, платье, обувь) — делай его СОВРЕМЕННЫМ. "
                f"ВАЖНО: сохрани все украшения и аксессуары с исходного фото. Если на фото нет колец — НЕ дорисовывай кольца. "
                f"Размер: {img_size}. "
            )
            if mode == "retry":
                prompt += " Сделай ДРУГОЙ вариант. Не повторяй предыдущий результат. "
...
async def handle_start(message: Message):
    ...
    await message.answer_photo(
        ...
        caption=(
            ...
            "📸 <b>Разбор фото:</b> бесплатно, 5 раз в день. Покажу ошибки композиции прямо на снимке.\n\n"
...
def _build_analysis_prompt(user_id: int) -> str:
    ...
    parts.append(
        "ДОРИСОВКА ОДЕЖДЫ (если обрезана краем кадра): "
        "Если ноги не полностью в кадре — дорисуй нижнюю часть современно: "
        "брюки прямые, свободные, широкие джинсы, чиносы, палаццо — НЕ скинни, НЕ клёш, НЕ варёнки. "
        "Платье прямого силуэта, миди, свободное — НЕ короткое, НЕ обтягивающее. "
        "Обувь современная: кроссовки, лоферы, челси, ботинки, аккуратные туфли. "
        "НЕ устаревшая, НЕ старомодная. "
        "Сохрани ВЕРХНЮЮ часть одежды как на исходном фото — НЕ меняй её. "
        "Если верх рубашка — низ брюки или джинсы. "
        "Если платье — продолжи его современным силуэтом. "
        "Обувь — по контексту: если сцена на улице, набережной, в городе — обувь обязательна. "
        "Если сцена домашняя или пляжная — допустимо босиком. "
...
async def handle_confirm_full(callback: CallbackQuery):
    ...
    gen_wish[user_id] = (
        ...
        f"ВЕРХНЮЮ одежду сохрани как на исходном фото. "
        f"Если дорисовываешь низ (брюки, платье, обувь) — делай его СОВРЕМЕННЫМ: "
        f"брюки прямые, свободные, широкие джинсы, чиносы, палаццо — НЕ скинни, НЕ клёш. "
        f"Платье прямого силуэта, миди, свободное. "
        f"Обувь современная: кроссовки, лоферы, челси, ботинки. "
        f"Обувь — по контексту: если сцена на улице, набережной — обувь обязательна. "
...
async def handle_gen_go_repose(callback: CallbackQuery):
    ...
    user_id = int(parts[4])
    gen_wish[user_id] = (
        "Полностью измени позу: разверни корпус, измени руки и ноги. Сохрани лицо и верхнюю одежду. "
        "Если дорисовываешь низ (брюки, платье, обувь) — делай его СОВРЕМЕННЫМ: "
        "брюки прямые, свободные, широкие джинсы, чиносы, палаццо — НЕ скинни, НЕ клёш. "
        "Платье прямого силуэта, миди, свободное. "
        "Обувь современная: кроссовки, лоферы, челси. "
        "Обувь — по контексту: если сцена на улице — обувь обязательна. "
        "Верхнюю часть одежды НЕ меняй."
    )
    await callback.answer("Запускаю генерацию...")
    await do_generation(user_id, callback.message.chat.id, gen_type)
    user_mode[user_id] = "free"
...
async def handle_gen_boost(callback: CallbackQuery):
    ...
    base_rules = (
        ...
        "ВЕРХНЮЮ одежду сохрани в точности. "
        "Если дорисовываешь низ (брюки, платье, обувь) — СОВРЕМЕННО: "
        "брюки прямые, свободные, широкие джинсы, чиносы — НЕ скинни, НЕ клёш. "
        "Платье прямого силуэта, миди. "
        "Обувь — по контексту: на улице — обувь, дома — можно без. "
