...
FLAT_LAY_PROMPTS = {
    "cozy": (
        ...
        "Полностью замени фон на тёплый деревянный стол с ЕСТЕСТВЕННОЙ, умеренной текстурой. "
        "Добавь ЛЁГКИЙ КОНТРАСТ: мягкие тени от предметов, естественные световые переходы. "
        "Избегай крайностей: не слишком тёмный, не слишком светлый, не слишком контрастный. "
        "Всё должно выглядеть натурально и естественно. "
        "РАСПОЗНАЙ, ЧТО ЗА ПРЕДМЕТЫ, и подбери декор ИМЕННО под эту тематику. "
        "НЕ добавляй чай или кофе, если предметы не связаны с едой. "
    ...
    "nature": (
        ...
        "Полностью замени фон на светлый мрамор или светлое дерево. "
        "РАСПОЗНАЙ, ЧТО ЗА ПРЕДМЕТЫ, и добавь уместные природные элементы. "
        "НЕ добавляй еду или напитки, если это не еда. "
        "Для обуви — листья, камень; для спорта — зелень, вода; для косметики — цветы. "
        "РАССТАВЬ предметы гармонично. "
        "Мягкий дневной свет. "
        "Сохрани все предметы."
    ),
    "dark": (
        "Создай ЭЛЕГАНТНЫЙ ТЁМНЫЙ Flat Lay как из Pinterest. "
        "Полностью замени фон на тёмный матовый или тёмное дерево. "
        "РАСПОЗНАЙ, ЧТО ЗА ПРЕДМЕТЫ, и подбери тёмный декор под тематику. "
        "НЕ добавляй напитки, если это не еда. "
        "РАССТАВЬ предметы стильно. "
        "Драматичный свет. "
        "Сохрани все предметы."
    ),
    "pastel": (
        "Создай НЕЖНЫЙ ПАСТЕЛЬНЫЙ Flat Lay как из Pinterest. "
        "Полностью замени фон на пастельный. "
        "РАСПОЗНАЙ, ЧТО ЗА ПРЕДМЕТЫ, и добавь нежный декор под тематику. "
        "НЕ добавляй еду или напитки, если это не еда. "
        "РАССТАВЬ предметы красиво. "
        "Мягкий свет. "
        "Сохрани все предметы."
    ),
}
...
async def handle_gen_go_retouch(callback: CallbackQuery):
    ...
    gen_type = parts[3]
    user_id = int(parts[4])
    gen_wish[user_id] = "Сделай полную профессиональную ретушь лица и кожи."
    await callback.answer("Запускаю генерацию...")
    await do_generation(user_id, callback.message.chat.id, gen_type)
    user_mode[user_id] = "free"
