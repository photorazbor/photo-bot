...
async def do_generation(user_id: int, chat_id: int, gen_type: str):
    ...
    try:
        ...
        prompt = f"Улучши это фото как опытный ретушёр. Сделай кадр гармоничным и естественным. Дорисуй обрезанные края — особенно конечности: если руки, ноги, пальцы обрезаны краем кадра, дорисуй их. Если ноги выглядят обрезанными ниже колен или отсутствуют — дорисуй голени и стопы. Исправь неестественную позу: если плечо прижато к телу и выглядит массивным — сделай его легче и изящнее, если таз и плечи на одной линии — добавь лёгкий разворот для объёма. Убери отвлекающие объекты, улучши свет и цвета. НЕ меняй черты лица — сохрани их в точности как на исходном фото. Сохрани все важные детали и объекты. Размер: {img_size}."
